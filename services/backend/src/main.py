...
Initialises the FastAPI application with a lifespan context that starts and gracefully stops the background cleanup scheduler. Also registers:
...
app.add_middleware(
    ...
    # allow_credentials must be True when the frontend sends an Authorization header with a Bearer token.
    # Without this, the browser blocks the request during the CORS preflight check.
...
# Registered first so they are always reachable, even if something below fails to initialise.
# These endpoints are the entry point to the entire app.
app.include_router(auth_router)

# Admin routes: /admin/stats, /admin/users, etc.
# Registered before upload_router so admin paths are matched before the generic root StaticFiles mount at the end catches everything.
...
# Any URL not matched by a router or earlier mount falls through to here, which lets the browser load the correct HTML file directly.
